...
from models.Fed import FedAvg,Fedrop,WeightDisGet, FedReplace
...
#         net_D.weight_init(mean=0.0, std=0.02)
    ...
    Cw_glob = net_G.state_dict()
    Cw_glob = net_D.state_dict()
...
#     fix_noise = torch.randn(64, 100, 1, 1).to(args.device)
    ...
    for iter in range(args.epochs):
        Cw_locals,Gw_locals,Dw_locals = [], [], []
        loss_Glocals, loss_Dlocals,loss_Clocals = [], [], []
        idxs_users = np.random.choice(range(args.num_users), m, replace=False)
#         if iter > 1:
#             WeightDisC = WeightDisGet(localCNet_dic,idxs_users,net_C.state_dict())
#             WeightDisG = WeightDisGet(localGNet_dic,idxs_users,net_G.state_dict())
#             WeightDisD = WeightDisGet(localDNet_dic,idxs_users,net_D.state_dict())
...
#             if iter > 1:
#                 Cw, Gw, Dw, Gloss, Dloss, Closs = local.GAN_train(C=copy.deepcopy(net_C).to(args.device),G=copy.deepcopy(net_G).to(args.device), D=copy.deepcopy(net_D).to(args.device),args=args, img_size=img_size, sampled_time=localnum_dic[idx],wd_c = WeightDisC[idx],wd_d = WeightDisD[idx],wd_g = WeightDisG[idx])
#             else:
            Cw, Gw, Dw, Gloss, Dloss, Closs = local.GAN_train(C=copy.deepcopy(net_C).to(args.device),G=copy.deepcopy(net_G).to(args.device), D=copy.deepcopy(net_D).to(args.device),args=args, img_size=img_size, sampled_time=localnum_dic[idx])
...
#             w_locals.append(copy.deepcopy(w))
        ...
        if iter > 0:
                Cw_glob = net_C.state_dict()
#                 print(Cw_glob.keys())
                Gw_glob = net_G.state_dict()
                Dw_glob = net_D.state_dict()
                localCNet_dic,Cw_locals = FedReplace(localCNet_dic,idxs_users,Cw_glob)
                localGNet_dic,Gw_locals = FedReplace(localGNet_dic,idxs_users,Gw_glob)
                localDNet_dic,Dw_locals = FedReplace(localDNet_dic,idxs_users,Dw_glob)
    ...
    workbook.save(r'./fedreplace_{}_{}_{}_{}.xlsx'.format(args.dataset,args.iid,args.label_rate,Time))
...
#         torch.save(net_G,'./save_net/generator/Gmodel_mnist.pkl')
        torch.save(net_D.state_dict(),'/code/FLtripleGAN/save_net/discriminator/Dmodelmnist.pkl')
        torch.save(net_C.state_dict(),'/code/FLtripleGAN/save_net/classifier/Cmodelmnist.pkl')
        torch.save(net_G.state_dict(),'/code/FLtripleGAN/save_net/generator/Gmodelmnist.pkl')

    elif args.dataset == 'svhn':
        torch.save(net_D,'/code/FLtripleGAN/save_net/discriminator/Dmodelsvhn.pkl')
        torch.save(net_C,'/code/FLtripleGAN/save_net/classifier/Cmodelsvhn.pkl')
        torch.save(net_G,'/code/FLtripleGAN/save_net/generator/Gmodelsvhn.pkl')


    elif args.model == 'mlp':
        torch.save(net_D,'/code/FLtripleGAN/save_net/discriminator/Dmodel_mlp.pkl')
        torch.save(net_C,'/code/FLtripleGAN/save_net/classifer/Cmodel_svhn_mlp.pkl')
        torch.save(net_G,'/code/FLtripleGAN/save_net/generator/Gmodel_mlp.pkl')
