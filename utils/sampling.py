...
def iid_sample(dataset,label_rate, num_users):
    """
...
def noniid_i_sample(dataset, label_rate, num_users=10, ppt = 0.5, Distrib = 'fixed'):
    '''
    :param dataset: dataset param
    :param label_rate: percentage of labeled data
    :param num_users: only works for 10 clients setting
...
def sample(dataset, num_users, label_rate, iid_if='iid'):
    ...
    if iid_if == 'noniid_ssl':
        for i in range(num_users):
            if i< 100 * label_rate:
                label_rate_dict[i] = (105 * label_rate -5) / (100 * label_rate) #0.55
            else:
    ...
    for i in range(num_users):
        ...
        #print('b',len(dict_users[i]), len(dict_users_labeled[i]))
...
def get_dict(dict_idxs_class, num_users, iid_if, label_rate, i):
    ...
    elif iid_if == 'noniid_tradition':
        print("warning! the client numbers of noniid_tradition should be 100")
        x, y = i//10, i%10
        if x < y:
            rep_x, rep_y = x+y+1, x
        elif x == y:
...
def mnist_noniid(dataset, num_users, label_rate):
    ...
    for i in range(num_users):
        ...
        for rand in rand_set:
            ...
            dict_users_labeled[i] = set(np.random.choice(list(dict_users[i]), int(num_items * label_rate), replace=False))
        dict_users_labeled[i] = set(dict_users[i])-set(dict_users_labeled[i])
    return dict_users, dict_users_labeled, dict_users_unlabeled
